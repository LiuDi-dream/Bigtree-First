...
### 💡 避坑指南与注意事项
* **队伍配置**：不同任务可以配置不同的队伍。
* **Boss 讨伐高危预警**：强烈建议使用拥有挂机输出且生存能力极强的队伍（如：少机、少莉、芙茜爱、四神等）。否则成功率难以保障（作者本人使用 6芙的莉希机芙，仍偶尔被深罪浸礼者拍死）。如讨伐经常失败，请联系对应脚本作者，或自行修改对应脚本json文件中的定位（比如三角铁）
...
*(测试连通性：可运行 `test_llm_confi.py` 测试大模型，运行 `skills/env_reader.py` 测试展柜数据读取（单独测试读取的时候，需要单独修改env_reader.py中257行的默认uid，不然无法拉取你自己的号的配置）)*
